...
KChat.prototype = {
	...
	init: function(){
		...
		this.socket.on('connect',function(){
			document.getElementById('info').textContent = '欢迎来到express+socket聊天室';
		...
		this.socket.on('loginFailed', function(){
			...
			info.textContent = '该呢称已存在，请重新输入新的呢称';
			info.style.color = 'red';
		});

		//登录成功
		this.socket.on('loginSuccess', function(){
			document.title = '聊天室成员(' + document.getElementById('nicknameInput').value + ')';
		...
		document.getElementById('loginBtn').addEventListener('click', function(){
			...
			if (nickname.length == 0 || nickname.length > 10){
				//如果用户名为空或超出长度限制
				info.textContent = '呢称不能为空，并且不能超过10个字符';
				info.style.color = 'red';
				nicknameInput.value = '';
				nicknameInput.focus();
			}else{
